...
import {main as retweetHandler} from '@functions/retweet/handler'
import fs from 'fs'
...
export const we_invoke_retweet = async ({
  username,
  tweetId,
}: {
  username: string
  ...
  return retweetHandler(event, context as unknown as Context, error => {
    if (error) {
      console.log(JSON.stringify(error))
    }
  })
